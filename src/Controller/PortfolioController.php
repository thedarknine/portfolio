...
final class PortfolioController extends AbstractController
{
    ...
    #[Route('/experience/{job}/{slug}', name: 'app_experience_slug')]
    // Sitemap generated by EventSubscriber\SitemapSubscriber
    public function experienceSlug(string $job, string $slug, ExperienceRepository $experienceRepository): Response
    {
        $page       = $this->pageService->getActivePageBySlug($slug);
        $experience = $experienceRepository->findOneBy(['slug' => $job]);

        if (!$experience) {
            throw $this->createNotFoundException('Experience not found');
        }

        return $this->render('pages/experience/' . $slug . '.html.twig', [
            'current_page' => $page,
            'breadcrumb'   => [
                'level1' => [
                    'url'   => $this->generateUrl('app_experience'),
                    'icon'  => 'hugeicons:briefcase-04',
                    'label' => 'Experience',
                ],
                'level2' => [
                    'url'   => $this->generateUrl('app_experience') . '#' . $job,
                    'icon'  => 'hugeicons:binoculars',
                    'label' => $experience->getCompany()->getName(),
                ],
            ],
        ]);
    }
